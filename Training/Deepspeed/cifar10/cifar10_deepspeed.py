...
def main(args):
    ...
    # env_list = ['SLURM_NTASKS','SLURM_NODEID','SLURM_LOCALID','SLURM_PROCID','SLURM_NTASKS','MASTER_PORT',]
    ...
    #     print("get_accelerator().device_name():",get_accelerator().device_name())
...
if __name__ == "__main__":
    ...
    # os.environ['MASTER_ADDR'] = "10.120.20.167"
    # os.environ['MASTER_PORT'] = "30001"
    os.environ['LOCAL_RANK'] = local_rank
    
    print("test_ZHANG:",os.getenv("ZHANG"))
    print("---node_id---device count:----",torch.cuda.device_count())
    ...
    print("-"*10,"slurm info","-"*10)
    env_list = ['SLURM_NTASKS','SLURM_NODEID','SLURM_LOCALID','SLURM_PROCID','SLURM_NTASKS','MASTER_PORT',]

    for env in env_list:
        print(f"value {env}:",os.getenv(env,None))
    # start training
    main(args)

    print("global rank: torch.distributed.get_rank():",torch.distributed.get_rank())
    print("torch.distributed.get_world_size():",torch.distributed.get_world_size())
    end_time = time.perf_counter()
    print("hostname:%s,cost time:%f s"%(hostname,round(end_time-start_time,3)))